...
async function saveProductToDB(product) {
  ...
  // ② رفع الصورة على Storage (اختياري — لو فشل نكمل بدونه)
  if (product.image && product.image.startsWith('data:')) {
    try {
      ...
      console.warn('⚠️ فشل رفع الصورة على Storage، هيتحفظ بدونها:', e.message);
      remoteProduct.image = ''; // مش هنوقف العملية بسببها
    }
  }
  ...
  try {
    await sbFetch('products', { method: 'POST', body: JSON.stringify(dbPayload) });
    showToast('✅ تم رفع المنتج للزبائن!', 'success');
    // حدّث IndexedDB بالـ URLs
    await idbPut('products', { ...product, image: remoteProduct.image, gallery: remoteProduct.gallery, files: product.files });
  } catch(e) {
