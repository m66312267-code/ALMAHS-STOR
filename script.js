...
function buildDetailHTML(p) {
  const isLocked = p.files_locked === true;
  ...
  const filesHTML = p.files && p.files.length ? `
    <div class="detail-files-section">
      ...
      <div class="detail-files-grid">
        ${p.files.map(f => `
          <div class="detail-file-card ${!canDownload ? 'locked' : ''}"
            onclick="${canDownload ? `downloadFile('${f.id}', '${f.name}', '${p.id}')` : `showToast('🔒 اشتري المنتج عشان تحمل الملفات', 'warn')`}"
            style="${!canDownload ? 'opacity:0.5;cursor:not-allowed;' : ''}">
            <div class="file-icon">${canDownload ? getFileIcon(f.name) : '🔒'}</div>
            <div class="file-info">
              <div class="file-name">${f.name}</div>
              <div class="file-size">${canDownload ? f.size : '—'}</div>
            </div>
          </div>
        `).join('')}
      </div>
...
// تحقق هل المستخدم اشترى المنتج ده (بناءً على الأوردرات المحفوظة)
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  reviews = revs;
  settings = { whatsapp: sett.whatsapp || '', passwordHash: sett.password_hash || '', accessCode: sett.access_code || '' };
  coupons = coups;
  cart = loadCartLocal();
