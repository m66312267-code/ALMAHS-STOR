...
async function loadProducts() {
  // دايماً حمّل من localStorage أولاً عشان الصور والملفات محفوظين فيه
  const local = JSON.parse(localStorage.getItem('local_products') || '[]');
  return local;
}
async function saveProductToDB(product) {
  // دايماً احفظ في localStorage أولاً كـ backup فوري
  try {
    let localProducts = JSON.parse(localStorage.getItem('local_products') || '[]');
    // حذف القديم لو موجود وزوده جديد
  ...
  } catch(le) { console.warn('local save error:', le); }

  // حاول ترفع على Supabase (اختياري — لو فشل مش مشكلة)
  try {
    // Supabase مش بيدعم base64 كبير في text columns — نرفع بدون data الملفات
    const productForDB = {
      ...product,
      files: (product.files || []).map(f => ({ id: f.id, name: f.name, size: f.size })),
      image: product.image ? '[local]' : '',
      gallery: []
    };
    ...
    console.warn('Supabase save failed (OK - saved locally):', e);
    return product;
  }
}
  ...
  grid.innerHTML = filtered.map(p => `
    <div class="product-card">
      <div class="product-img-wrap">
        ...
        <div class="product-badge" style="background:${p.status==='نفذ'?'rgba(239,68,68,0.85)':p.status==='قريباً'?'rgba(245,158,11,0.85)':'rgba(255,98,0,0.85)'};">${p.status || p.category || 'متاح'}</div>
