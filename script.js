...
function getFileIcon(name) {
  ...
  const icons = { pdf: '📄', doc: '📝', docx: '📝', xls: '📊', xlsx: '📊', zip: '🗜️', rar: '🗜️', png: '🖼️', jpg: '🖼️', jpeg: '🖼️', gif: '🖼️', mp4: '🎬', mov: '🎬', mp3: '🎵', psd: '🎨', ai: '🎨', fig: '🎨', html: '💻', css: '💻', js: '💻' };
  return icons[ext] || '📁';
}
  ...
  list.innerHTML = [...orders].reverse().map(o => `
    <div class="admin-order-item">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
        <div style="font-weight:700;color:var(--text);">أوردر # ${o.id.substr(0, 8)}</div>
        <span style="background:rgba(124,58,237,0.12);color:var(--accent-2);padding:4px 12px;border-radius:100px;font-size:12px;border:1px solid rgba(124,58,237,0.2);">${o.status}</span>
      </div>
      ${o.customerName ? `<div style="margin-bottom:6px;font-size:14px;color:var(--text-sub);">👤 ${o.customerName} &nbsp;|&nbsp; 📞 ${o.customerPhone || 'غير محدد'}</div>` : ''}
      <div style="margin-bottom:10px;color:var(--text-dim);font-size:13px;">📅 ${o.date}</div>
      <div style="margin-bottom:12px;">
        ${o.items.map(i => `<div style="font-size:13px;color:var(--text-sub);margin-bottom:4px;">• ${i.name} × ${i.quantity} = ${formatPrice(i.price * i.quantity)}</div>`).join('')}
      </div>
      <div style="font-weight:700;font-size:15px;background:linear-gradient(90deg,var(--accent-2),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">الإجمالي: ${formatPrice(o.total)}</div>
    </div>
  `).join('');
}
