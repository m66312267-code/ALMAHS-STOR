...
// ============================================================
// ★ SUPABASE HEALTH — تشخيص فوري لمشاكل الاتصال والصلاحيات
// ============================================================
let _sbCanWrite = null; // null=لم يُختبر، true=يعمل، false=فاشل

async function testSupabaseWrite() {
  // اختبار سريع: هل نقدر نكتب على settings؟
  try {
    const payload = { id: 1 };
    ...
    _sbCanWrite = res.ok || res.status === 404; // 404 = الصف مش موجود بس الصلاحية شغالة
    return { ok: _sbCanWrite, status: res.status, statusText: res.statusText };
  } catch(e) {
    _sbCanWrite = false;
...
async function runSupabaseDiagnostic() {
  const banner = document.getElementById('sbDiagBanner');
  if (banner) banner.remove();

  showToast('🔍 جارٍ اختبار الاتصال...', 'default');

  // اختبار القراءة
  let canRead = false, readStatus = '';
  try {
    const res = await fetch(SUPABASE_URL + '/rest/v1/products?limit=1', {
      headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + SUPABASE_KEY }
    });
    canRead = res.ok;
    readStatus = res.status + ' ' + res.statusText;
  } catch(e) { readStatus = 'خطأ: ' + e.message; }

  // اختبار الكتابة
  const writeResult = await testSupabaseWrite();
  const canWrite = writeResult.ok;
  const writeStatus = writeResult.error || (writeResult.status + ' ' + (writeResult.statusText || ''));

  // اختبار الإضافة — نتجنب INSERT عشان الحذف بعده صعب
  // بنختبر بـ PATCH بدل INSERT
  let canInsert = false, insertStatus = '';
  try {
    const res = await fetch(SUPABASE_URL + '/rest/v1/products?id=eq.nonexistent_test_id', {
      method: 'PATCH',
      headers: {
        'apikey': SUPABASE_KEY,
        'Authorization': 'Bearer ' + SUPABASE_KEY,
      ...
      body: JSON.stringify({ name: '__patch_test__' })
    });
    // 200 أو 204 = الصلاحية شغالة (حتى لو مفيش row اتعدّل)
    canInsert = res.ok;
    insertStatus = res.status + ' ' + res.statusText;
  } catch(e) { insertStatus = 'خطأ: ' + e.message; }

  // عرض النتيجة
  const div = document.createElement('div');
  div.id = 'sbDiagBanner';
  div.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,0.7);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:16px;';

  const allOk = canRead && canWrite && canInsert;
  div.innerHTML = `
    <div style="background:var(--bg-card,#1a1a1a);border:1px solid var(--border,rgba(255,255,255,0.1));border-radius:20px;padding:28px 24px;max-width:480px;width:100%;font-family:Cairo,sans-serif;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
        <div style="font-size:32px;">${allOk ? '✅' : '⚠️'}</div>
        <div>
          <div style="font-weight:900;font-size:18px;color:var(--text,#fff);">تشخيص Supabase</div>
          <div style="font-size:13px;color:var(--text-sub,#aaa);">نتيجة اختبار الاتصال والصلاحيات</div>
        </div>
      </div>

      <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px;">
        <div style="background:${canRead ? 'rgba(34,160,70,0.1)' : 'rgba(239,68,68,0.1)'};border:1px solid ${canRead ? 'rgba(34,160,70,0.3)' : 'rgba(239,68,68,0.3)'};border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:10px;">
          <span style="font-size:20px;">${canRead ? '✅' : '❌'}</span>
          <div style="flex:1;">
            <div style="font-weight:700;color:var(--text,#fff);font-size:14px;">القراءة (SELECT)</div>
            <div style="font-size:12px;color:var(--text-dim,#888);">${readStatus}</div>
          </div>
        </div>

        <div style="background:${canWrite ? 'rgba(34,160,70,0.1)' : 'rgba(239,68,68,0.1)'};border:1px solid ${canWrite ? 'rgba(34,160,70,0.3)' : 'rgba(239,68,68,0.3)'};border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:10px;">
          <span style="font-size:20px;">${canWrite ? '✅' : '❌'}</span>
          <div style="flex:1;">
            <div style="font-weight:700;color:var(--text,#fff);font-size:14px;">التعديل (UPDATE/PATCH)</div>
            <div style="font-size:12px;color:var(--text-dim,#888);">${writeStatus}</div>
          </div>
        </div>

        <div style="background:${canInsert ? 'rgba(34,160,70,0.1)' : 'rgba(239,68,68,0.1)'};border:1px solid ${canInsert ? 'rgba(34,160,70,0.3)' : 'rgba(239,68,68,0.3)'};border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:10px;">
          <span style="font-size:20px;">${canInsert ? '✅' : '❌'}</span>
          <div style="flex:1;">
            <div style="font-weight:700;color:var(--text,#fff);font-size:14px;">الإضافة (INSERT)</div>
            <div style="font-size:12px;color:var(--text-dim,#888);">${insertStatus}</div>
          </div>
        </div>
      </div>

      ${!allOk ? `
      <div style="background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.3);border-radius:10px;padding:14px 16px;margin-bottom:16px;">
        <div style="font-weight:700;color:#f59e0b;font-size:14px;margin-bottom:6px;">🔧 الحل:</div>
        <div style="font-size:13px;color:var(--text-sub,#aaa);line-height:1.7;">
          ${!canRead ? '❶ الاتصال بـ Supabase فاشل كلياً — تحقق من الـ URL والـ API Key<br>' : ''}
          ${canRead && (!canWrite || !canInsert) ? `❶ افتح <strong style="color:#f59e0b;">Supabase Dashboard</strong><br>❷ روح <strong>Authentication → Policies</strong><br>❸ على كل جدول (products, settings, orders) اضغط <strong>"New Policy"</strong><br>❹ اختر <strong>"Enable read and write access for all users"</strong><br>❺ احفظ وجرب تاني` : ''}
        </div>
      </div>
      ` : `
      <div style="background:rgba(34,160,70,0.1);border:1px solid rgba(34,160,70,0.3);border-radius:10px;padding:14px 16px;margin-bottom:16px;">
        <div style="font-weight:700;color:#22a046;font-size:14px;">🎉 كل حاجة شغالة! لو التغييرات لسه مش بتتحفظ — تواصل معانا.</div>
      </div>
      `}

      <button onclick="document.getElementById('sbDiagBanner').remove()" style="width:100%;background:linear-gradient(135deg,#22a046,#ff8c00);color:#fff;border:none;padding:12px;border-radius:10px;font-family:Cairo,sans-serif;font-weight:700;font-size:15px;cursor:pointer;">حسناً، فهمت</button>
    </div>
  `;
...
// ============================================================
// ★ SUPABASE STORAGE — رفع الصور والملفات (بدل base64 ضخم)
// ============================================================
async function uploadToStorage(bucket, filePath, base64Data) {
  // حوّل base64 لـ Blob
...
// ضغط الصورة قبل الرفع (يوفر وقت ومساحة)
function compressImage(base64, maxW = 1200, quality = 0.82) {
  return new Promise(resolve => {
    ...
    img.onerror = () => resolve(base64); // لو فشل الضغط، خلي الأصلي
    img.src = base64;
  });
}
...
async function flushSyncQueue() {
  ...
  // احذف العمليات القديمة أكتر من 24 ساعة (مش هتنفع)
  const oneDayAgo = Date.now() - (24 * 60 * 60 * 1000);
  for (const item of items) {
    if (item.timestamp && item.timestamp < oneDayAgo) {
      await idbDelete('syncQueue', item.qid).catch(() => {});
    }
  }
  items = await idbGetAll('syncQueue');
  if (!items.length) return;
  ...
  for (const item of items) {
    try {
      // تنظيف files_meta من البيانات القديمة قبل الرفع
      if (item.data) {
        delete item.data.files_meta;
        if (item.data.files) {
          item.data.files = item.data.files.map(f => ({ id: f.id, name: f.name, size: f.size, url: f.url || '' }));
        }
      }
      if (item.type === 'upsert') {
        await sbFetch(item.table, { method: 'POST', body: JSON.stringify(item.data), headers: { 'Prefer': 'resolution=merge-duplicates,return=representation' } });
      ...
      // لو فشل 3 مرات أو أكتر، احذفه من الـ queue عشان ما يعيقش
      const retries = (item.retries || 0) + 1;
      if (retries >= 3) {
        await idbDelete('syncQueue', item.qid).catch(() => {});
        console.warn(`⚠️ تم حذف العملية ${item.qid} بعد 3 محاولات فاشلة`);
      } else {
        await idbPut('syncQueue', { ...item, retries }).catch(() => {});
      }
    }
  }
...
async function loadProductsRemote() {
  // نجرب عدة طرق للجلب — من الأكمل للأبسط — عشان نتجنب خطأ "column does not exist"
  const queries = [
    'products?select=id,name,description,price,category,image,gallery,files,created_at,user_id,files_locked,status,pinned,hidden&order=created_at.desc',
    'products?select=id,name,description,price,category,image,gallery,created_at,user_id,files_locked,status,pinned,hidden&order=created_at.desc',
    'products?select=id,name,description,price,category,image,gallery,created_at,status,pinned,hidden&order=created_at.desc',
    'products?select=id,name,description,price,category,image,gallery,created_at&order=created_at.desc',
  ...
  for (const q of queries) {
    try {
      ...
      if (res && Array.isArray(res)) { data = res; break; }
    } catch(e) { console.warn('Query failed, trying simpler:', e.message); }
  }
  if (!data) { console.warn('All Supabase attempts failed, using local cache'); return null; }

  // تجاهل المنتجات اللي اتحذفت محلياً
  try {
    const deletedRec = await idbGet('meta', 'deleted_ids').catch(() => null);
    const deletedList = (deletedRec && deletedRec.value) ? deletedRec.value : [];
    if (deletedList.length > 0) {
      data = data.filter(p => !deletedList.includes(p.id));
    }
  } catch(e) {}
  // حفظ في IndexedDB مع المحافظة على الـ files المحلية
  for (const p of data) {
    // لو في files_meta قديمة، حوّلها للشكل المعتاد (للتوافق مع البيانات القديمة)
    if (p.files_meta && !p.files) {
      p.files = p.files_meta;
    }
    delete p.files_meta; // ازل files_meta من كل مكان
    try {
...
async function loadProductFull(id) {
  try {
    ...
    if (fresh && fresh[0]) {
      const p = fresh[0];
      // توافق مع بيانات قديمة كانت تستخدم files_meta
      if (p.files_meta && !p.files) p.files = p.files_meta;
      delete p.files_meta;
      await idbPut('products', p);
...
async function saveProductToDB(product) {
  ...
  // ② رفع الصورة على Storage مع إعادة المحاولة 3 مرات
  if (product.image && product.image.startsWith('data:')) {
    let imgUploaded = false;
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        showToast(`⏳ جارٍ رفع الصورة... (${attempt}/3)`, 'info');
        const compressed = await compressImage(product.image);
        const imgUrl = await uploadToStorage('products', `${product.id}/cover.jpg`, compressed);
        remoteProduct.image = imgUrl;
        imgUploaded = true;
        break;
      } catch(e) {
        console.warn(`⚠️ محاولة رفع الصورة ${attempt}/3 فشلت:`, e.message);
        if (attempt < 3) await new Promise(r => setTimeout(r, 1500 * attempt));
      }
    }
    if (!imgUploaded) {
      // الصورة مش اترفعت — احتفظ بالـ base64 في Supabase كـ fallback مؤقت
      console.warn('⚠️ فشل رفع الصورة 3 مرات — هيتحفظ بـ base64 مؤقتاً');
      remoteProduct.image = product.image; // base64 مؤقت عشان مش تبقى فاضية
      showToast('⚠️ فشل رفع الصورة — ستظهر بشكل مؤقت', 'warn');
    }
  }

  // ③ رفع صور الجاليري (اختياري)
  if (product.gallery && product.gallery.length) {
    const galleryUrls = [];
    for (let i = 0; i < product.gallery.length; i++) {
      const g = product.gallery[i];
      if (g && g.startsWith('data:')) {
        try {
          const compressed = await compressImage(g, 900, 0.80);
          const url = await uploadToStorage('products', `${product.id}/gallery_${i}.jpg`, compressed);
          galleryUrls.push(url);
        } catch(e) {
          console.warn('فشل رفع gallery صورة:', i);
        }
      } else {
        galleryUrls.push(g);
      }
    }
    remoteProduct.gallery = galleryUrls;
  }

  // ④ رفع الملفات على Storage (اختياري)
  if (product.files && product.files.length) {
    const remoteFiles = [];
    for (const f of product.files) {
      if (f.data && f.data.startsWith('data:')) {
        try {
          const fileUrl = await uploadToStorage('products', `${product.id}/files/${f.id}_${f.name}`, f.data);
          remoteFiles.push({ id: f.id, name: f.name, size: f.size, url: fileUrl });
        } catch(e) {
          console.warn('فشل رفع ملف:', f.name);
          remoteFiles.push({ id: f.id, name: f.name, size: f.size, url: '' });
        }
      } else {
        remoteFiles.push(f);
      }
    }
    remoteProduct.files = remoteFiles;
  }

  // ⑤ حفظ في Supabase — دايماً بيحصل حتى لو Storage فشل
  const dbPayload = { ...remoteProduct };
  dbPayload.files = remoteProduct.files
    ? remoteProduct.files.map(f => ({ id: f.id, name: f.name, size: f.size, url: f.url || '' }))
    : [];
  delete dbPayload.files_meta;

  try {
    // استخدام UPSERT بدل POST عشان لو المنتج موجود بالفعل ميعملش duplicate
    await sbFetch('products', { method: 'POST', body: JSON.stringify(dbPayload), headers: { 'Prefer': 'resolution=merge-duplicates,return=representation' } });
    showToast('✅ تم رفع المنتج للزبائن!', 'success');
    // حدّث IndexedDB بالـ URLs الصحيحة من Storage
    const updatedProduct = { ...product, image: remoteProduct.image, gallery: remoteProduct.gallery, files: product.files };
    await idbPut('products', updatedProduct);
    // حدّث الـ products array في الذاكرة بالـ URL الصح
    const idx = products.findIndex(p => p.id === product.id);
    if (idx !== -1) products[idx] = updatedProduct;
  } catch(e) {
    console.error('❌ Supabase DB save failed:', e.message);
    await queueOperation({ type: 'upsert', table: 'products', id: product.id, data: dbPayload });
    showToast('⚠️ حُفظ محلياً فقط — تحقق من إعدادات Supabase', 'warn');
  }
}
...
async function deleteProductFromDB(id) {
  ...
  // ② احفظ الـ ID في قائمة المحذوفات عشان نمنع رجوعه من Supabase
  try {
    const deletedRec = await idbGet('meta', 'deleted_ids').catch(() => null);
    const deletedList = (deletedRec && deletedRec.value) ? deletedRec.value : [];
    if (!deletedList.includes(id)) deletedList.push(id);
    await idbPut('meta', { key: 'deleted_ids', value: deletedList });
  } catch(e) {}

  // ③ حذف من Supabase مع 3 محاولات
  let done = false;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      await sbFetch('products?id=eq.' + id, { method: 'DELETE', prefer: 'return=minimal' });
      done = true;
      console.log('✅ تم حذف المنتج من Supabase، المحاولة ' + attempt);
      break;
    } catch(e) {
      console.warn('❌ محاولة حذف ' + attempt + '/3 فشلت:', e.message);
      if (attempt < 3) await new Promise(r => setTimeout(r, 1000 * attempt));
    }
  }
  if (!done) {
    await queueOperation({ type: 'delete', table: 'products', id });
    console.warn('🔄 تم وضع الحذف في queue للمحاولة لاحقاً');
  }
}
...
async function syncProductToSupabase(id) {
  ...
  const patchData = {
    ...
    image: p.image || '',
    gallery: p.gallery || [],
    files: p.files ? p.files.map(f => ({ id: f.id, name: f.name, size: f.size, url: f.url || '' })) : []
  };

  try {
    await sbFetch('products?id=eq.' + id, { method: 'PATCH', body: JSON.stringify(patchData), prefer: 'return=minimal' });
  } catch(e) {
    console.error('❌ Supabase product patch failed:', e.message);
    await queueOperation({ type: 'patch', table: 'products', id, data: patchData });
    _sbCanWrite = false;
    showToast('⚠️ التعديل حُفظ محلياً — Supabase مش شغال', 'warn');
  }
}
...
async function loadSettings() {
  try {
    ...
    const hasSocial = s.social && typeof s.social === 'object' && Object.values(s.social).some(v => v);
    if (!hasSocial) {
...
async function saveSettings() {
  ...
  try {
    ...
    try {
      await sbFetch('settings', { method: 'POST', body: JSON.stringify({ id: 1, ...payload }) });
      sbOk = true;
    } catch(e2) {
...
async function deleteAdminCategory(id) {
  customConfirm('هتحذف القسم ده؟ المنتجات مش هتتحذف.', async () => {
...
async function initPassword() {
  if (!settings.passwordHash) {
    // لو مفيش باسورد محفوظ، اطلب من الأدمن يضبط باسورد جديد
    const stored = localStorage.getItem('mahsmarket_pwd_hash');
    if (stored) {
      settings.passwordHash = stored;
      await saveSettings();
    }
    // مش بنضبط باسورد افتراضي في الكود — الأدمن يضبطه من الإعدادات
  }
...
async function verifyPassword(input) {
  ...
  // لو مفيش باسورد خالص — ارفض الدخول
  return false;
}
...
// ============================================================
// ============================================================
// ★ CUSTOM CONFIRM — بدل confirm() القبيح
// ============================================================
function customConfirm(message, onConfirm) {
  // ابحث أو اصنع الـ modal
  let modal = document.getElementById('customConfirmModal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'customConfirmModal';
    modal.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,0.6);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;padding:16px;';
    modal.innerHTML = `
      <div style="background:var(--bg-card,#fff);border:1px solid var(--border,rgba(34,160,70,0.2));border-radius:18px;padding:28px 24px;max-width:360px;width:100%;text-align:center;box-shadow:0 24px 80px rgba(0,0,0,0.2);">
        <div style="font-size:40px;margin-bottom:14px;">⚠️</div>
        <div id="customConfirmMsg" style="font-size:16px;font-weight:700;color:var(--text,#0d2e17);margin-bottom:24px;line-height:1.6;"></div>
        <div style="display:flex;gap:10px;justify-content:center;">
          <button id="customConfirmNo" style="flex:1;background:rgba(255,255,255,0.08);color:var(--text-sub,#2d6b3f);border:1px solid var(--border,rgba(34,160,70,0.2));padding:11px;border-radius:10px;font-family:Cairo,sans-serif;font-size:14px;font-weight:700;cursor:pointer;">إلغاء</button>
          <button id="customConfirmYes" style="flex:1;background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;border:none;padding:11px;border-radius:10px;font-family:Cairo,sans-serif;font-size:14px;font-weight:700;cursor:pointer;">تأكيد</button>
        </div>
      </div>`;
  ...
  document.getElementById('customConfirmYes').onclick = () => { close(); onConfirm(); };
  document.getElementById('customConfirmNo').onclick = close;
...
async function downloadFile(fileId, fileName, productId) {
  ...
  // لو الملف عنده URL من Storage، حمّله مباشرة
  if (f && f.url) {
  ...
  // لو مش موجود، ابدأ بالبحث في IndexedDB
  if (!f) {
  ...
  // لو لسه مش موجود، اجلبه من Supabase
  if (!f) {
    try {
      ...
      if (freshData && freshData[0]) {
        ...
        // جرب files_meta لو موجودة
        const meta = freshData[0].files || freshData[0].files_meta;
        f = meta ? meta.find(x => x.id === fileId) : null;
      }
    } catch(e) {
  ...
  reader.onload = async (e) => {
    try {
      ...
      const product = {
        ...
        image: e.target.result, // base64 مؤقتاً — saveProductToDB هيرفعها لـ Storage
      ...
      // حدّث الـ product في الـ array بالـ URLs الصح (بعد ما saveProductToDB حدّثها)
      const savedProduct = products.find(p => p.id === product.id) || product;
...
async function deleteProduct(id) {
  customConfirm('هتحذف المنتج ده؟ مش هيرجع!', async () => {
    try {
...
async function saveEditProduct() {
  ...
  const applyChanges = async (imgData) => {
    p.name = name; p.price = price; p.description = desc;
    p.category = category; p.status = status; p.adminNote = adminNote;
    if (imgData) {
      // ارفع الصورة الجديدة على Storage لو base64
      if (imgData.startsWith('data:')) {
        try {
          showToast('⏳ جارٍ رفع الصورة...', 'info');
          const compressed = await compressImage(imgData);
          const imgUrl = await uploadToStorage('products', `${p.id}/cover.jpg`, compressed);
          p.image = imgUrl;
        } catch(uploadErr) {
          console.warn('image upload failed, saving base64 locally:', uploadErr);
          p.image = imgData;
        }
      } else {
        p.image = imgData;
  ...
  if (imageFile) {
    const reader = new FileReader();
    reader.onload = async e => { await applyChanges(e.target.result); };
    reader.readAsDataURL(imageFile);
  } else { await applyChanges(null); }
}
...
async function confirmOrder() {
  ...
  if (waNumber) {
    const storeName = (settings.aboutInfo && settings.aboutInfo.title) || 'ALMAHSMarket';
    const couponLine = couponCode ? `\n🏷️ *كوبون:* ${couponCode} (وفّر ${formatPrice(appliedCoupon.discount)})` : '';

    let msg;
    if (settings.orderMsg && settings.orderMsg.trim()) {
      // ★ استخدم رسالة الأدمن المخصصة مع استبدال المتغيرات
      msg = settings.orderMsg
        .replace(/{name}/g, name)
        .replace(/{phone}/g, phone)
        .replace(/{product}/g, p.name)
        .replace(/{price}/g, formatPrice(finalTotal))
        .replace(/{coupon}/g, couponCode || 'لا يوجد')
        .replace(/{store}/g, storeName);
      // أضف تفاصيل الكوبون لو موجود ومش متضمن في الرسالة المخصصة
      if (couponCode && !settings.orderMsg.includes('{coupon}')) {
        msg += couponLine;
      }
    } else {
      // الرسالة الافتراضية — باسم المتجر من الإعدادات
      msg = `🛒 *طلب جديد — ${storeName}*\n\n👤 الاسم: ${name}\n📞 التليفون: ${phone}\n\n📦 *المنتج:* ${p.name}${couponLine}\n💰 *الإجمالي: ${formatPrice(finalTotal)}*\n\nأرجو التأكيد، شكراً! 🙏`;
    }
  ...
  closeOrderModal();
  renderAdminOrders();
  showToast('✅ ' + getOrderConfirmMsg(name, p.name), 'success');
}
...
async function deleteReview(id) {
  customConfirm('هتحذف الرأي ده؟', async () => {
    try {
...
async function deleteOrder(id) {
  customConfirm('هتحذف الأوردر ده؟', async () => {
    try {
...
function loadAdminSettings() {
  ...
  // رسالة الترحيب
  safeSet('settingsWelcomeTitle', settings.welcomeTitle);
  safeSet('settingsWelcomeMsg', settings.welcomeMsg);
  // رسالة تأكيد الطلب
  safeSet('settingsOrderMsg', settings.orderMsg);
  // معلومات المتجر
  const a = settings.aboutInfo || {};
  safeSet('settingsStoreName', a.title);
  safeSet('settingsStoreEmail', a.email);
  safeSet('settingsStoreIcon', a.icon);
  safeSet('settingsStoreSubtitle', a.subtitle);
  safeSet('settingsStoreAbout', a.about);
  safeSet('settingsStoreFeatures', (a.features || []).join(', '));
...
async function saveAccessCode() {
  ...
  // نحفظ الكود فقط بدون ما نمسح باقي الإعدادات
  try {
    await sbFetch('settings?id=eq.1', { method: 'PATCH', body: JSON.stringify({ access_code: code }), prefer: 'return=minimal' });
  } catch(e) { console.warn('saveAccessCode failed:', e.message); }
...
async function saveAdminSettings() {
  ...
  // رسالة الترحيب
  settings.welcomeTitle = safeGet('settingsWelcomeTitle');
  settings.welcomeMsg = safeGet('settingsWelcomeMsg');

  // رسالة تأكيد الطلب
  settings.orderMsg = safeGet('settingsOrderMsg');

  // معلومات المتجر
  const featuresRaw = safeGet('settingsStoreFeatures');
  settings.aboutInfo = {
    title: safeGet('settingsStoreName'),
    email: safeGet('settingsStoreEmail'),
    icon: safeGet('settingsStoreIcon'),
    subtitle: safeGet('settingsStoreSubtitle'),
    about: safeGet('settingsStoreAbout'),
...
// ============================================================
// ★ APPLY STORE SETTINGS — رسالة الترحيب ومعلومات المتجر
// ============================================================
function applyStoreSettings() {
  ...
  if (a.title) {
    document.title = a.title + (a.subtitle ? ' — ' + a.subtitle : '');
  }

  // ② النص في شريط التنقل (nav logo)
  if (a.title) {
    const navLogoSpan = document.querySelector('.nav-logo > span');
    const navLogoFull = document.querySelector('.nav-logo');
    if (navLogoSpan) {
      navLogoSpan.textContent = a.title;
    } else if (navLogoFull) {
      // لو الـ span مش موجود، غير النص الأخير في الـ logo
      const textNodes = [...navLogoFull.childNodes].filter(n => n.nodeType === 3);
      if (textNodes.length) textNodes[textNodes.length - 1].textContent = '';
      navLogoSpan && (navLogoSpan.textContent = a.title);
    }
  }

  // ③ hero badge ("مرحبا بك في ...")
  if (a.title) {
    ...
    if (heroBadge) {
      ...
      heroBadge.appendChild(document.createTextNode(' مرحبا بك في ' + a.title));
    }
  }

  // ④ رسالة الترحيب على الصفحة الرئيسية
  const titleEl = document.getElementById('heroSectionTitle');
  const subEl   = document.getElementById('heroSectionSub');
  if (titleEl && settings.welcomeTitle) titleEl.textContent = settings.welcomeTitle;
  if (subEl) {
    if (settings.welcomeMsg) subEl.textContent = settings.welcomeMsg;
    else if (a.subtitle && !settings.welcomeMsg) subEl.textContent = a.subtitle;
  }

  // ⑤ footer — اسم صاحب المتجر / copyright
  const footerCopy = document.querySelector('.footer-copy');
  if (footerCopy && a.title) {
    const year = new Date().getFullYear();
    footerCopy.textContent = `© ${year} ${a.title} — جميع الحقوق محفوظة`;
  }
...
function getOrderConfirmMsg(customerName, productName, price, phone) {
  // رسالة التوست دايماً مختصرة — الرسالة الكاملة بتروح واتساب
  const storeName = (settings.aboutInfo && settings.aboutInfo.title) || 'ALMAHSMarket';
  return `شكراً ${customerName}! طلبك على "${productName}" اتسجل بنجاح ✅ هنتواصل معاك على واتساب في أقرب وقت 🚀`;
}
...
function openWaChat() {
  ...
  const storeName = (settings.aboutInfo && settings.aboutInfo.title) || 'ALMAHSMarket';
  window.open(`https://wa.me/${waNumber}?text=${encodeURIComponent('أهلاً! مهتم بخدماتكم في ' + storeName + ' 👋')}`, '_blank');
}
...
function initWaFloating() {
  ...
  // ★ تم إيقاف الفتح التلقائي — الزبون يفتحها بنفسه
  waAutoShown = true;
}
...
async function deleteCoupon(id) {
  customConfirm('هتحذف الكوبون ده؟', async () => {
...
function handleFilesInput(files) {
  ...
  Array.from(files).forEach(file => {
    ...
    reader.onload = (ev) => {
      ...
      added++;
      renderPendingFiles();
      if (added === 1) showToast('✅ جاري رفع ' + total + ' ملف...', 'success');
    };
...
async function deleteUserProject(projectId) {
  ...
  customConfirm('هتحذف المشروع ده؟', async () => {
    try {
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  settings = {
    ...
    welcomeTitle: (sett && sett.welcome_title) || '',
    welcomeMsg: (sett && sett.welcome_msg) || '',
    orderMsg: (sett && sett.order_msg) || ''
  };
  coupons = coups || [];
  ...
  // ④ جلب المنتجات من Supabase — مع timeout 12 ثانية
  const remoteTimeout = setTimeout(() => {
    // لو الـ fetch اتأخر أكتر من 12 ثانية وعندنا منتجات محلية، ابقى على المحلية
    const grid = document.getElementById('productsGrid');
    if (grid && grid.innerHTML.includes('جارٍ التحميل') && products.length === 0) {
      grid.innerHTML = '<div style="text-align:center;padding:60px;color:var(--text-dim);grid-column:1/-1;"><div style="font-size:56px;margin-bottom:16px;">📦</div><p>لا توجد منتجات حتى الآن</p><p style="font-size:12px;margin-top:8px;opacity:0.6;">تحقق من الاتصال بالإنترنت</p></div>';
    }
  }, 12000);
  ...
  loadProductsRemote().then(remoteProds => {
    ...
    if (remoteProds && remoteProds.length > 0) {
      ...
      renderProducts();
      renderAdminProducts(); // ★ مهم: حدّث لوحة الأدمن كمان
      if (hadDiff) console.log('✅ تم تحديث المنتجات من Supabase');
    } else if (products.length === 0) {
      // Supabase رجع فاضي والكاش فاضي
      const grid = document.getElementById('productsGrid');
      if (grid) grid.innerHTML = '<div style="text-align:center;padding:60px;color:var(--text-dim);grid-column:1/-1;"><div style="font-size:56px;margin-bottom:16px;">📦</div><p>لا توجد منتجات حتى الآن</p></div>';
    }
  }).catch(e => {
    clearTimeout(remoteTimeout);
    console.warn('Background product sync failed:', e);
    if (products.length === 0) {
      const grid = document.getElementById('productsGrid');
      if (grid) grid.innerHTML = '<div style="text-align:center;padding:60px;color:var(--text-dim);grid-column:1/-1;"><div style="font-size:56px;margin-bottom:16px;">⚠️</div><p>تعذر تحميل المنتجات</p><p style="font-size:12px;margin-top:8px;opacity:0.6;">تحقق من الاتصال وأعد تحميل الصفحة</p></div>';
    }
  });
...
// checkUserPanel called inside main DOMContentLoaded above
