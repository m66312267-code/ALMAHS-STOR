...
async function runSupabaseDiagnostic() {
  ...
  try {
    ...
    const res = await fetch(SUPABASE_URL + '/rest/v1/products', {
      ...
      body: JSON.stringify({ id: testId, name: '__test__', description: 'test', price: 1, created_at: new Date().toISOString(), hidden: false, status: 'متاح', pinned: false })
    });
    canInsert = res.ok || res.status === 409; // 409 = conflict = الصلاحية شغالة
    insertStatus = res.status + ' ' + res.statusText;
    // احذف الـ test row لو اتضاف
    if (res.ok) {
      await fetch(SUPABASE_URL + '/rest/v1/products?id=eq.' + testId, {
        method: 'DELETE',
...
async function saveProductToDB(product) {
  // ① حفظ فوري في IndexedDB (بالبيانات الكاملة محلياً)
  await idbPut('products', product);

  // ② رفع الصورة الرئيسية لـ Storage لو كانت base64
  ...
  try {
    ...
    // ④ رفع الملفات لـ Storage (بدل تخزينها كـ base64 في DB)
    ...
    // ⑤ حفظ في Supabase بدون base64 (خفيف وسريع)
    const dbPayload = { ...remoteProduct };
    // احفظ metadata الملفات فقط (اسم، حجم، url) في عمود files — بدون base64
    dbPayload.files = remoteProduct.files
      ? remoteProduct.files.map(f => ({ id: f.id, name: f.name, size: f.size, url: f.url || '' }))
      : [];
    // تأكد مفيش files_meta عشان العمود ده مش موجود في Supabase
    delete dbPayload.files_meta;

    await sbFetch('products', { method: 'POST', body: JSON.stringify(dbPayload) });

    // حدّث الـ IndexedDB بالـ URLs الجديدة
    const updatedProduct = { ...product, image: remoteProduct.image, gallery: remoteProduct.gallery, files: product.files }; // نحتفظ بـ data محلياً
