...
function handleFilesInput(files) {
  ...
  Array.from(files).forEach(file => {
    ...
    if (pendingFiles.some(f => f.name === file.name && f.size === file.size)) {
      showToast('❌ الملف موجود بالفعل', 'error');
      return;
    }
  ...
  renderPendingFiles();
  showToast(`✅ تم إضافة ${files.length} ملف`, 'success');
}
  ...
  el.innerHTML = pendingFiles.map((f, i) => `
    <div style="display:flex;align-items:center;gap:10px;padding:10px;background:rgba(255,98,0,0.08);border-radius:8px;margin-bottom:8px;border-left:3px solid #ff6200;">
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:11px;color:var(--text-dim);margin-top:3px;">${(f.size/1024/1024).toFixed(2)} MB</div>
      </div>
