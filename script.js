...
// LOCAL STORAGE
// ============================================================
function saveProducts() { localStorage.setItem('mahsmarket_products', JSON.stringify(products)); }
function loadProducts() { const s = localStorage.getItem('mahsmarket_products'); return s ? JSON.parse(s) : []; }
function saveOrders() { localStorage.setItem('mahsmarket_orders', JSON.stringify(orders)); }
function loadOrders() { const s = localStorage.getItem('mahsmarket_orders'); return s ? JSON.parse(s) : []; }
function saveReviews() { localStorage.setItem('mahsmarket_reviews', JSON.stringify(reviews)); }
function loadReviews() { const s = localStorage.getItem('mahsmarket_reviews'); return s ? JSON.parse(s) : []; }
function saveCart() { localStorage.setItem('mahsmarket_cart', JSON.stringify(cart)); }
function loadCart() { const s = localStorage.getItem('mahsmarket_cart'); return s ? JSON.parse(s) : []; }
function saveSettings() { localStorage.setItem('mahsmarket_settings', JSON.stringify(settings)); }
function loadSettings() { const s = localStorage.getItem('mahsmarket_settings'); return s ? JSON.parse(s) : { whatsapp: '', passwordHash: '' }; }
...
async function initPassword() {
  if (!settings.passwordHash) {
    // Default password: 2008 — يُرجى تغييرها من الإعدادات
  ...
  const filesHTML = p.files && p.files.length ? `
    <div class="detail-files-section">
      ...
      <div class="detail-files-grid">
        ${p.files.map(f => `
          <div class="detail-file-card" onclick="downloadFile('${f.id}', '${f.name}')">
    ...
    <div class="detail-inner">
      <div class="detail-grid">
        <div class="detail-gallery">
          ${p.image ? `<img class="detail-main-img" src="${p.image}" alt="${p.name}">` : `<div class="detail-main-placeholder">📦</div>`}
...
function addProduct() {
  ...
  reader.onload = (e) => {
    const product = { id: generateId(), name, price, desc, category, image: e.target.result, files: [...pendingFiles] };
...
function updateCartUI() {
  ...
  // Keep a hidden cartTotal for checkout
...
// ============================================================
// CHECKOUT MODAL (customer info + WhatsApp)
// ============================================================
...
function submitReview() {
  ...
  reviews.push({ id: generateId(), name, product, text, stars: selectedStars, approved: false, date: Date.now() });
  saveReviews();
  updateRevTabCount();
...
function approveReview(id) {
  ...
  if (r) { r.approved = true; saveReviews(); renderReviews(); renderAdminReviews(); updateRevTabCount(); showToast('✅ تم نشر الرأي!', 'success'); }
}

function deleteReview(id) {
  if (!confirm('هتحذف الرأي ده؟')) return;
  reviews = reviews.filter(r => r.id !== id);
  saveReviews(); renderReviews(); renderAdminReviews(); updateRevTabCount(); showToast('🗑️ تم حذف الرأي', 'warn');
}
...
// 5 نقرات سريعة على اللوجو تفتح مربع كود الوصول
...
function handleLogoTap(e) {
  ...
  if (logoTapCount >= 5) {
    ...
    if (!code) return;
    const saved = localStorage.getItem('mahsmarket_access_code');
    if (saved && code === saved) {
...
async function checkPwd() {
  ...
  if (ok) {
    ...
    showAdminBtn(); // يظهر الزر بعد تسجيل الدخول
    closePwdModal();
    openAdmin();
  } else {
...
function renderAdminOrders() {
  ...
  list.innerHTML = [...orders].reverse().map(o => {
    ...
    const itemsText = o.items.map(i => `• ${i.name} × ${i.quantity} = ${formatPrice(i.price * i.quantity)}`).join('%0A');
    const waMsg = encodeURIComponent(`👋 أهلاً ${o.customerName}،\nبخصوص طلبك رقم #${o.id.substr(0,6)} من ALMAHSMarket\n\n📦 ${o.items.map(i=>`${i.name} × ${i.quantity}`).join(', ')}\n💰 الإجمالي: ${formatPrice(o.total)}\n\nتفضل كلمنا هنساعدك 😊`);
    ...
    <div class="admin-order-item" id="order-${o.id}">
      ...
      <div style="background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:12px;">
        <div style="font-size:14px;font-weight:700;color:var(--text);margin-bottom:6px;">👤 ${o.customerName}</div>
        <div style="font-size:13px;color:var(--text-sub);">📞 <span dir="ltr">${o.customerPhone || 'غير محدد'}</span></div>
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        ${phone ? `
          <a href="https://wa.me/${phone}?text=${waMsg}" target="_blank"
            style="flex:1;min-width:120px;display:flex;align-items:center;justify-content:center;gap:8px;padding:10px;border-radius:10px;background:rgba(37,211,102,0.12);color:#25d366;border:1px solid rgba(37,211,102,0.3);font-weight:700;font-size:13px;text-decoration:none;transition:all 0.2s;">
          ...
          <a href="tel:${phone}"
            style="padding:10px 16px;border-radius:10px;background:rgba(240,180,41,0.1);color:var(--gold);border:1px solid rgba(240,180,41,0.25);font-weight:700;font-size:13px;text-decoration:none;display:flex;align-items:center;gap:6px;transition:all 0.2s;">
            📞 اتصال
          </a>` : '<span style="font-size:12px;color:var(--text-dim);">⚠️ مفيش رقم تليفون</span>'}
        <button onclick="deleteOrder('${o.id}')"
          style="padding:10px 14px;border-radius:10px;background:rgba(239,68,68,0.08);color:#ef4444;border:1px solid rgba(239,68,68,0.2);font-weight:700;font-size:13px;cursor:pointer;font-family:inherit;transition:all 0.2s;">
          🗑️
        </button>
      </div>
    </div>`;
...
function loadAdminSettings() {
  ...
  document.getElementById('currentAccessCode').textContent = saved ? saved : '(مش محدد)';
}
...
function initWaFloating() {
  ...
  // Auto-show bubble after 4 seconds (once only)
  if (!waAutoShown) {
    setTimeout(() => {
...
function saveCoupons() { localStorage.setItem('mahsmarket_coupons', JSON.stringify(coupons)); }
function loadCoupons() { const s = localStorage.getItem('mahsmarket_coupons'); return s ? JSON.parse(s) : []; }
...
function addCoupon() {
  ...
  coupons.push({ id: generateId(), code, value, type, active: true, usedCount: 0 });
  saveCoupons();
  renderCouponsList();
...
function toggleCoupon(id) {
  ...
  if (c) { c.active = !c.active; saveCoupons(); renderCouponsList(); }
}
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  await initPassword();
  checkAdminAccess(); // إظهار زر الأدمن لو في صلاحية
