...
async function loadProducts() {
  try {
    ...
    if (data && data.length > 0) {
      // حفظ نسخة خفيفة (بدون base64) في localStorage
  ...
  // fallback على local cache
  return JSON.parse(localStorage.getItem('local_products') || '[]');
}
async function saveProductToDB(product) {
  // حفظ نسخة خفيفة في localStorage كـ cache
  try {
...
async function downloadFile(fileId, fileName, productId) {
  ...
  // لو الـ data مش موجودة (اتحذفت من cache)، اجلبها من Supabase
  if (!f || !f.data) {
    try {
      const freshData = await sbFetch('products?id=eq.' + productId);
      if (freshData && freshData[0]) {
        const fp = freshData[0];
        const idx = products.findIndex(x => x.id === productId);
        if (idx !== -1) products[idx] = { ...products[idx], ...fp };
        f = fp.files ? fp.files.find(x => x.id === fileId) : null;
      }
    } catch(e) {
...
function handleLogoTap(e) {
  ...
  if (logoTapCount >= 5) {
    ...
    if (!saved) {
      // مفيش كود محدد — افتح باسورد الإدمن مباشرة
...
window.addEventListener('DOMContentLoaded', async () => {
  // تنظيف localStorage القديم لو فيه base64 ضخمة
  try {
    const oldCache = localStorage.getItem('local_products');
    if (oldCache && oldCache.length > 1000000) localStorage.removeItem('local_products');
  } catch(e) {}

  const grid = document.getElementById('productsGrid');

  // ① عرض الكاش الفوري — المنتجات تظهر فوراً بدون انتظار
  const cachedProds = JSON.parse(localStorage.getItem('local_products') || '[]');
  ...
  // ② تحميل الإعدادات أولاً (خفيفة وسريعة)
  const sett = await loadSettings().catch(() => ({ id: 1, whatsapp: "201063786533", password_hash: "", access_code: "ADMIN2024" }));

  // ③ باقي البيانات بالتوازي
  const [prods, ords, revs, coups] = await Promise.all([
    loadProducts().catch(() => cachedProds),
    loadOrders().catch(() => []),
    loadReviews().catch(() => []),
    loadCoupons().catch(() => [])
  ]);
